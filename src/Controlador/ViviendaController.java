...
public class ViviendaController extends Conexion {
    ...
    public ViviendaDAO Get(int Escritura) throws SQLException {
        String sql = "SELECT Escritura,TipoViviendaID,TipoVivienda.Nombre AS NombreTipoVivienda,\n"
                + "Dirección,MunicipioID, Municipio.Nombre AS NombreMunicipio,DepartamentoID\n"
                + "FROM Vivienda\n"
                + "INNER JOIN TipoVivienda\n"
                + "ON Vivienda.TipoViviendaID = TipoVivienda.ID\n"
                + "INNER JOIN Municipio\n"
                + "ON Vivienda.MunicipioID = Municipio.ID  WHERE Escritura =?";
        PreparedStatement oPreparedStatement = conecta().prepareStatement(sql);
        oPreparedStatement.setInt(1, Escritura);
        ResultSet rs = oPreparedStatement.executeQuery();

        if (rs.next()) {
            return new ViviendaDAO(rs.getInt("Escritura"),rs.getInt("TipoViviendaID"),
                    new TipoViviendaDAO(rs.getInt("TipoViviendaID"),rs.getString("NombreTipoVivienda")),rs.getString("Dirección"), rs.getInt("MunicipioID"),
                    new MunicipioDAO(rs.getInt("MunicipioID"),rs.getString("NombreMunicipio"),rs.getInt("DepartamentoID")));
        }
        return null;
    }
    ...
    public ArrayList<ViviendaDAO> Get() throws SQLException {
        ...
        String sql = "SELECT Escritura,TipoViviendaID,TipoVivienda.Nombre AS NombreTipoVivienda,\n"
                ...
                + "                Departamento.ID AS DepartamentoID, Departamento.Nombre AS NombreDepartamento,DepartamentoID\n"
                + "                FROM Vivienda\n"
                + "                INNER JOIN TipoVivienda\n"
                + "                ON Vivienda.TipoViviendaID = TipoVivienda.ID\n"
                + "                INNER JOIN Municipio\n"
